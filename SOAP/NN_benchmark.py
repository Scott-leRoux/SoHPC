...
t0 = time.time()
model, history = train_model(model, D_train, E_train, 32, 500, D_val, E_val)
t1 = time.time()
with open('nn_bench.txt','a') as f:
	if gpu:
		f.write(f"GPU time: {t1-t0}")
		print(f"GPU time: {t1-t0}")
	else:
		f.write(f"CPUS: {N_CPUS}, time: {t1-t0}")
		print(f"CPUS: {N_CPUS}, time: {t1-t0}")
