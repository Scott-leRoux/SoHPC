...
for i in range(1,5):
	D_train.append(np.load(f"cm_qm9_train{i}.npy"))
...
D_test = np.load("cm_qm9_test.npy")
D_val = np.load("cm_qm9_train0.npy")
...
def model_builder(hp):
	...
	if hp_act_fn == 'softplus':
		model.add(Dense(hp.Choice(f'layer_{0}_units', values = [256,512,128])))
		model.add(layers.Activation('softplus'))
	elif hp_act_fn == 'ssp':
		model.add(Dense(hp.Choice(f'layer_{0}_units', values = [256, 512, 128]), activation = shifted_softplus))
	else:
		model.add(Dense(hp.Choice(f'layer_{0}_units', values = [256, 512, 128]), kernel_initializer = 'he_uniform'))
...
class BatchTuner(kt.tuners.RandomSearch):
	def run_trial(self, trial, *args, **kwargs):
		kwargs['batch_size'] = trial.hyperparameters.Choice('batch_size', [32,64,128])
		super(BatchTuner, self).run_trial(trial,*args, **kwargs)

tuner_batch = BatchTuner(model_builder,
                        objective = 'val_loss',
                        executions_per_trial = 1,
                        max_trials = 1,
                        directory = 'kt_results',
...
tuner_batch.search(D_train_stck, E_train_stck, callbacks = callbacks, validation_data = (D_val, E_val), epochs = 100, verbose = 1)
best_hps=tuner_batch.get_best_hyperparameters()[0].values
print(best_hps)
print(tuner_batch.results_summary())
print(tuner_batch.get_best_models()[0].summary())	
